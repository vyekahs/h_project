...
/**
 * Pick a random layout for the given difficulty and adjust to required count.
...
export function pickTemplate(difficulty: string, requiredCount: number): TilePosition[] {
	...
	let positions = generateLayout(spec.baseCols, spec.baseRows, spec.layers, seed, shapeFn);

	// If the shape produced too few tiles (< 12), fall back to rectangle
	if (positions.length < 12) {
		positions = generateLayout(spec.baseCols, spec.baseRows, spec.layers, seed, rectangle);
	}

	// Adjust to required count
	if (positions.length > requiredCount) {
		// Trim from top layers first
		positions.sort((a, b) => b.layer - a.layer || b.row - a.row || b.col - a.col);
		positions = positions.slice(positions.length - requiredCount);
		// Ensure still multiple of 3
		while (positions.length % 3 !== 0) {
			positions.pop();
		}
	}
