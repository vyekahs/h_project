...
# 새 코드가 기동하기 전에 스키마를 먼저 맞춘다. 빌드보다 앞이라 실패를 가장 빨리 잡는다.
#
# 앱 컨테이너도 기동 시 같은 마이그레이션을 적용한다(start.sh → migrate_sql_files.js).
# 그래도 여기서 먼저 돌리는 이유는 실패를 빌드 전에 잡기 위해서다 — 기동 시에만
# 적용하면 새 슬롯이 크래시 루프에 빠지고 헬스체크 타임아웃까지 기다려야 원인을 안다.
#
# 이 시점엔 기존 활성 슬롯(구버전 코드)이 트래픽을 받고 있으므로, 마이그레이션은
# 구버전과도 호환되는 형태(컬럼 추가 등 additive)여야 한다. 컬럼/테이블 삭제처럼
# 구버전을 깨뜨리는 변경은 배포를 두 번에 나눠야 안전하다.
# 실패하면 여기서 중단되어 트래픽 전환까지 가지 않는다(set -e).
